...
import { telegramImageBotWorkflow } from "./workflows/telegramImageBotWorkflow";
...
export const mastra = new Mastra({
  ...
  workflows: { telegramImageBotWorkflow },
  mcpServers: {
    ...
    apiRoutes: [
      ...
      ...registerTelegramTrigger({
        ...
        handler: async (mastra: Mastra, triggerInfo: any) => {
          ...
          const threadId = `telegram/${triggerInfo.payload?.message?.chat?.id || 'unknown'}`;
          
          logger?.info("📝 [Telegram Trigger] Starting workflow", { threadId });

          // Start the image generation workflow
          const run = await mastra.getWorkflow("telegramImageBotWorkflow").createRunAsync();
